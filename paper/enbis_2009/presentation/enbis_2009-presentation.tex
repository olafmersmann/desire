...
\setupfootertexts[{%
    ...
    \hfill
    \llap{\tfx\FooterFg ENBIS 2009 / {\tt desiRe}}
}]
...
%\showmakeup
\TitlePage{%
  \Huge Multicriteria optimization using R and the desirability package {\tt desire}
  \vfill
  \tfx ENBIS 2009 \\
  \tfx Olaf Mersmann,
  Heike~Trautmann,
...
\startslide[(Noisy) Multicriteria Optimization Problem]
  \startblock[Given:]
...
\startslide[Transformed problem]
  \startblock[Given:]
    \def\arraystretch{1.5}
    \starttable[|l|l|]
      \NC Influence factors \NC $X=(X_1,\cdots, X_n)'$ \NC\FR
  ...
  \startblock[Goal:]
    \startformula
      \min_{X_1,\cdots,X_n} (d_1(Y_1),\cdots, d_k(Y_k))
    \stopformula
  \stopblock
\stopslide
